...
type KanaCharacters = {
  [kana: string]: string[]; // Each character maps to an array of strings (readings)
};


// Define the full structure of the "SimpleKanaDictionary"
type KanaDictionary = {
  [key in KanaType]: {
...
export const SimpleKanaDictionary: KanaDictionary = {
  'hiragana': {
    'h_group1': {'あ': ['a'], 'い': ['i'], 'う': ['u'], 'え': ['e'], 'お': ['o']},
    'h_group2': {'か': ['ka'], 'き': ['ki'], 'く': ['ku'], 'け': ['ke'], 'こ': ['ko']},
    'h_group3': {'さ': ['sa'], 'し': ['shi', 'si'], 'す': ['su'], 'せ': ['se'], 'そ': ['so']},
    'h_group4': {'た': ['ta'], 'ち': ['chi', 'ti'], 'つ': ['tsu', 'tu'], 'て': ['te'], 'と': ['to']},
    'h_group5': {'な': ['na'], 'に': ['ni'], 'ぬ': ['nu'], 'ね': ['ne'], 'の': ['no']},
    'h_group6': {'は': ['ha'], 'ひ': ['hi'], 'ふ': ['fu', 'hu'], 'へ': ['he'], 'ほ': ['ho']},
    ...
    'h_group12_a': {'ざ': ['za'], 'じ': ['ji', 'zi'], 'ず': ['zu', 'du'], 'ぜ': ['ze'], 'ぞ': ['zo']},
    'h_group13_a': {'だ': ['da'], 'ぢ': ['ji', 'di', 'dzi'], 'づ': ['zu', 'dzu'], 'で': ['de'], 'ど': ['do']},
    'h_group14_a': {'ば': ['ba'], 'び': ['bi'], 'ぶ': ['bu'], 'べ': ['be'], 'ぼ': ['bo']},
    'h_group15_a': {'ぱ': ['pa'], 'ぴ': ['pi'], 'ぷ': ['pu'], 'ぺ': ['pe'], 'ぽ': ['po']},
    'h_group16_a': {'きゃ': ['kya'], 'きゅ': ['kyu'], 'きょ': ['kyo']},
    'h_group17_a': {'しゃ': ['sha', 'sya'], 'しゅ': ['shu', 'syu'], 'しょ': ['sho', 'syo']},
    'h_group18_a': {'ちゃ': ['cha', 'cya', 'tya'], 'ちゅ': ['chu', 'cyu'], 'ちょ': ['cho', 'cyo']},
    ...
    'h_group24_a': {'じゃ': ['ja', 'jya'], 'じゅ': ['ju', 'jyu'], 'じょ': ['jo', 'jyo']},
